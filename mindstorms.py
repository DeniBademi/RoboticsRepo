...
class Motor(CoppeliaComponent):
    ...
    def __init__(self, motor_port, direction, clientID):
        ...
        """
               ...
               Args:
                   motor_port: The motor port, either A or B (that's applicable only for the coppelia sim).
                   direction (Direction): The direction of the motor rotation, either CLOCKWISE or COUNTERCLOCKWISE.
               """
    ...
    def run(self, speed):
        ...
        if self.motor_port == 'A':
            self.__set_speed(speed * self.direction.value, 0)
        elif self.motor_port == 'B':
            self.__set_speed(0, self.direction.value * speed)
