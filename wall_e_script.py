...
if clientID != -1:
    ...
    while True:
        # your code goes here
        set_speed(50, 50)
        img = get_image_top_cam()
        height, width, channels = img.shape
        top_left_quarter = img[:int(height / 2), :int(width / 2), :]
        top_right_quarter = img[:int(height / 2), int(width / 2):, :]
        bottom_left_quarter = img[int(height / 2):, :int(width / 2), :]
        bottom_right_quarter = img[int(height / 2):, int(width / 2):, :] 
        
        #green_channel = top_left_quarter[:, :, 1]
        #red_channel = top_left_quarter[:, :, 0]
        #show_image(green_channel)
        #print(red_channel.__class__)
        """
        if 255 in red_channel:
            index = np.where(red_channel == 255, red_channel)
            if green_channel[index] == 255:
                print("yellow detected") 
        """
        for i in top_left_quarter:
            for j in i:
                if top_left_quarter[i,j,0] == 255 and top_left_quarter[i,j,1] == 255:
                    print("yellow detected") 
    ...
    print('Failed connecting to remote API server')
print('Program ended')
>>>>>>> 63a759c33034ef1df1ac0883e0ffd94107d04246
